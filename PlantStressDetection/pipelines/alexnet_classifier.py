...
from utils.plot_style import apply_dark_style, PALETTE, GREEN, BLUE, YELLOW, RED, DARK_BG, PANEL_BG, BORDER, TEXT, MUTED
apply_dark_style()

from utils.data_utils import get_dataloaders
from utils.metrics import calculate_classification_metrics, save_metrics

class AlexNetKerasPort(nn.Module):
    def __init__(self, num_classes=38):
        super(AlexNetKerasPort, self).__init__()
        
        self.features = nn.Sequential(
            # Convolution Step 1
            nn.Conv2d(3, 96, kernel_size=11, stride=4, padding=0),  # padding 'valid'
            ...
            # Convolution Step 2
            nn.Conv2d(96, 256, kernel_size=11, stride=1, padding=0),
            nn.ReLU(inplace=True),
            # Max Pooling Step 2
            nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
            nn.BatchNorm2d(256),
        ...
        self.classifier = nn.Sequential(
            # Flattening Step implicitly done in forward
            ...
            # Final output layer
            # Do NOT add Softmax here since nn.CrossEntropyLoss handles it implicitly
...
def freeze_bottom_layers(model):
    # As per prompt:
    # freeze the first 8 layers of keras.
    # In PyTorch:
    # 0 conv2d_1
    # 1 max_pooling2d_1
    ...
    # We will freeze up to the first 8 keras layers which correspond to the first few PyTorch layers.
    # PyTorch features:
    # 0: Conv2d(96)
    # 1: ReLU
    # 2: MaxPool2d
    ...
    # 9: ReLU
    # 10: BatchNorm2d(384)
    # Here, Keras layers: (0, 1, 2) is block 1. (3, 4, 5) is block 2. (6, 7) is Conv3 + BN.
    # So we freeze up to index 10 in PyTorch self.features.
...
def run_alexnet_pipeline(config_path="config.yaml"):
