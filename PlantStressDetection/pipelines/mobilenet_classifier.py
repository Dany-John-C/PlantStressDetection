...
def get_mobilenet_model(num_classes, pretrained=True):
...
def unfreeze_top_layers(model, unfreeze_ratio=0.3):
    """
    Unfreezes the top `unfreeze_ratio` fraction of layers in the feature extractor.
    """
...
def run_mobilenet_pipeline(config_path="config.yaml"):
