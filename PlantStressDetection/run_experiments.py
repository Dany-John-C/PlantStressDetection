...
def main():
    ...
    parser.add_argument("--only", nargs="+", choices=["p1","p2","p3","p4","p5","p6","cross","robust","deploy","paper"],
    ...
    # ── Pipelines ──────────────────────────────────────────────────────────────
    if should_run("p1"):
        from pipelines.texture_svm import run_texture_svm
        t0 = time.perf_counter()
        try:
    ...
    if should_run("p2"):
        from pipelines.mobilenet_classifier import run_mobilenet_pipeline
        t0 = time.perf_counter()
        try:
    ...
    if should_run("p3"):
        from pipelines.severity_regression import run_regression_pipeline
        t0 = time.perf_counter()
        try:
    ...
    if should_run("p4"):
        from pipelines.embedding_anomaly import run_anomaly_pipeline
        t0 = time.perf_counter()
        try:
    ...
    if should_run("p5"):
        from pipelines.entropy_drift import run_entropy_drift
        t0 = time.perf_counter()
        try:
    ...
    if should_run("p6"):
        from pipelines.alexnet_classifier import run_alexnet_pipeline
        t0 = time.perf_counter()
        try:
    ...
    if should_run("paper"):
        ...
        t0 = time.perf_counter()
        try:
            console.print(Rule("[bold yellow]Generating Research Paper[/bold yellow]", style="yellow"))
            generate_paper()
            d = time.perf_counter() - t0
            console.print(f"  [bold green]✓[/bold green] Paper written to [cyan]results/final_research_paper.md[/cyan] in {d:.1f}s\n")
